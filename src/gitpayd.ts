...
import setup, { GitpaydConfig, handlePaymentAction, PaymentAction } from './setup';
...
APP.get("/gitpayd/decode/:paymentRequest", (req, res) => {
    ...
    handlePaymentAction(req.params.paymentRequest, PaymentAction.DECODE, res)
        .then(decoded => res.status(GitpaydConfig.HTTP_OK).json({ amt: decoded.data.num_satoshis }))
        .catch(() => res.status(GitpaydConfig.SERVER_FAILURE).json({msg: 'gitpayd failed to decode payment request'}));
});
...
APP.get("/gitpayd/balance", (req, res) => {
    ...
    handlePaymentAction(null, PaymentAction.BALANCE, res)
    .then(balance => res.status(GitpaydConfig.HTTP_OK).json({ balance: balance.data.local_balance }))
    .catch(() => res.status(GitpaydConfig.SERVER_FAILURE).json({msg: 'gitpayd failed to send payment'}));
});
...
APP.post("/gitpayd/pay/:paymentRequest", (req, res) => {
    ...
    // send the payment request to the lnd node
    handlePaymentAction(req.params.paymentRequest, PaymentAction.PAY, res)
        .then(pay => res.status(GitpaydConfig.HTTP_OK).json( {image: pay.data.payment_preimage }))
        .catch(() => res.status(GitpaydConfig.SERVER_FAILURE).json({msg: 'gitpayd failed to send payment'}));
});
